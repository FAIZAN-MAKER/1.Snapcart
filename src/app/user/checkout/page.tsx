...
const CheckoutPage = () => {
  ...
  const handlePlaceOrder = async () => {
    if (!userData?._id) { setOrderError("User session expired. Please log in again."); return; }
    if (!address.fullAddress) { setOrderError("Please pin your delivery location on the map."); return; }
    ...
    try {
      const response = await axios.post("/api/user/order", orderData);

      // ── LOGGING ORDER TO CONSOLE ───────────────────────────────────────────
      ...
      console.error("❌ Order Placement Failed:", error.response?.data ?? err.message);
    } finally {
      setIsPlacingOrder(false);
    }
  };

  const isDisabled = isPlacingOrder || cartItems.length === 0 || !address.fullAddress || paymentMethod === "stripe";

  const containerVariants = { hidden: { opacity: 0 }, show: { opacity: 1, transition: { staggerChildren: 0.08 } } };
  const itemVariants = { hidden: { opacity: 0, y: 20 }, show: { opacity: 1, y: 0, transition: { type: "spring", stiffness: 100, damping: 16 } } };
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <motion.div variants={itemVariants} className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  <h2 className="font-bold text-gray-900 text-base">Delivery Details</h2>
                  <p className="text-xs text-gray-400">Confirm your contact info</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4">
                <Field label="Full Address" icon={MapPin} value={address.fullAddress}
                  onChange={(v) => setAddress((p) => ({ ...p, fullAddress: v }))} placeholder="Will be filled from map…"
                />
            ...
            <motion.div variants={itemVariants} className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  <h2 className="font-bold text-gray-900 text-base">Pin Your Location</h2>
                  <p className="text-xs text-gray-400">Drag the marker or search to set your delivery address</p>
                </div>
              </div>
            ...
            <motion.div variants={itemVariants} className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <PaymentBtn selected={paymentMethod === "stripe"} onClick={() => setPaymentMethod("stripe")}
                  icon={CreditCard} label="Pay Online" sublabel="Stripe — Cards, UPI & more"
                />
                <PaymentBtn selected={paymentMethod === "cod"} onClick={() => setPaymentMethod("cod")}
                  icon={Banknote} label="Cash on Delivery" sublabel="Pay when order arrives"
                />
              </div>
              <AnimatePresence>
                {paymentMethod === "stripe" && (
                  <motion.p initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                    className="text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded-xl px-4 py-2.5 mt-3"
                  >
                    ⚠️ Online payment coming soon. Please use Cash on Delivery for now.
                  </motion.p>
                )}
              </AnimatePresence>
            </motion.div>
          ...
          <motion.div variants={itemVariants} className="lg:col-span-5 sticky top-6">
            <div className="bg-gray-950 text-white rounded-[2.5rem] p-8 shadow-2xl overflow-hidden relative border border-white/5">
              ...
              <h2 className="text-xl font-bold mb-6 flex items-center gap-2 relative">
                <ShoppingBag className="w-5 h-5 text-green-400" /> Order Summary
              </h2>

              <div className="space-y-4 mb-6 max-h-48 overflow-y-auto pr-2 custom-scrollbar relative">
                {cartItems.map((item) => (
                  <div key={item._id?.toString()} className="flex items-center gap-4">
                    ...
                    <p className="text-sm font-bold shrink-0 text-white">Rs. {(parseFloat(item.price) * item.quantity).toFixed(0)}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-4 relative">
                ...
                <div className="flex justify-between">
                  ...
                  <span className={`font-bold ${deliveryFee === 0 ? "text-green-400" : "text-gray-200"}`}>
                ...
                <div className="flex justify-between items-end">
                  ...
                  <span className="text-3xl font-black text-white">Rs. {finalTotal.toFixed(0)}</span>
                </div>
                ...
                <motion.button
                  ...
                  className={`w-full py-4 rounded-2xl font-black text-base flex items-center justify-center gap-2 mt-1 transition-all ${
                    isDisabled
                      ...
                      : "bg-green-500 hover:bg-green-400 text-gray-950 shadow-xl shadow-green-500/20 active:shadow-none"
                  }`}
                >
                  {isPlacingOrder
                ...
                {!address.fullAddress && !orderError && (
                  <p className="text-center text-[10px] text-gray-500 font-bold uppercase tracking-tighter">Pin location to enable button</p>
                )}
                ...
                <div className="flex items-center justify-center gap-2 pt-1 opacity-50">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-wider">
                    {paymentMethod === "cod" ? "Cash on Delivery" : "Stripe Integration"}
                  </span>
                </div>
              </div>
            </div>
